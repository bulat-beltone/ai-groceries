...
Статическое SPA в `index.html`, данные в `localStorage` (`gr7`: позиции, история, счётчики, **категории**, флаги перевода, режим списка). **Экспорт / Импорт** JSON в шапке — тот же формат, что в `localStorage`; поддерживается обёртка `{ "gr7": "<строка>" }` как в дампах ключей хранилища.
